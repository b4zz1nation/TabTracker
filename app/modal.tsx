  ...
  const handleSave = async () => {
    ...
    if (interestEnabled) {
      if (!normalizedRate || parsedRate <= 0 || !interestType) {
        Alert.alert('Error', 'Please provide a valid interest rate and frequency.');
        return;
      }
    }
      ...
      <View className="mb-4 p-5 bg-white dark:bg-zinc-900 rounded-3xl border border-gray-100 dark:border-zinc-800 shadow-sm">
        <View className="flex-row items-center justify-between mb-6">
          <View className="flex-row items-center">
            ...
            <Text className="text-base font-bold text-gray-900 dark:text-gray-100">Charge Interest</Text>
          </View>
          <Switch
