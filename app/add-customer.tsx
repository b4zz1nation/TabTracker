...
export default function AddCustomerScreen() {
  ...
  const params = useLocalSearchParams<{ lendId?: string, customerId?: string }>();
      ...
      await db.withTransactionAsync(async () => {
        if (isEditing && params.lendId) {
          ...
          if (params.customerId || name.trim()) {
             await db.runAsync('UPDATE customers SET name = ? WHERE id = ?', [name.trim(), params.customerId ?? null]);
          }
          await db.runAsync(
    ...
    <SheetScreen onClose={() => router.back()} dragDirections={{ toBottom: true, toTop: false, toLeft: false, toRight: false }} opacityOnGestureMove={true} containerRadiusSync={true}>
      <KeyboardAvoidingView behavior={Platform.OS === 'ios' ? 'padding' : 'height'} className="flex-1">
        <SafeAreaView className="flex-1 bg-gray-50 dark:bg-gray-950" edges={['top']}>
          ...
          <Text className="text-3xl font-bold text-gray-900 dark:text-gray-100 mb-8">{isEditing ? 'Edit Lend' : 'New Customer'}</Text>
