...
export default function ProfileScreen() {
  ...
  return (
    <ScreenContainer header={<View className="px-5 py-4"><Text className="text-2xl font-bold text-gray-900 dark:text-gray-100">Profile</Text></View>}>
      <View className="items-center mt-10 p-6 bg-white dark:bg-gray-900 mx-6 rounded-[32px] shadow-sm border border-gray-100 dark:border-gray-800">
        ...
        <Text className="text-2xl font-black text-gray-900 dark:text-gray-100 mb-1">{user?.name || 'Guest User'}</Text>
        <Text className="text-gray-400 dark:text-gray-500 font-medium mb-6 uppercase tracking-widest text-[10px]">Personal Account</Text>
        ...
        <View className="w-full border-t border-gray-100 dark:border-gray-800 pt-6 gap-4">
          <TouchableOpacity className="flex-row items-center p-4 rounded-2xl bg-gray-50 dark:bg-gray-800/50">
            <Ionicons name="settings-outline" size={24} color="#9ca3af" className="mr-4" />
            <Text className="text-base font-bold text-gray-700 dark:text-gray-300">Settings</Text>
